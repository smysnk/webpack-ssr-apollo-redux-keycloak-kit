...
const load = async file => {
  ...
  let wp;
  try {
    wp = (await import(path.resolve(PATHS.webpack, file))).default;
  } catch (e) {
    console.error(`Error: ${ file } not found or has errors:`);
    console.error(e);
    process.exit();
  }
...
// Set the 'root' path to the 'webpack' dir in this folder
  ...
  for (const build of (process.env.WEBPACK_CONFIG || '').trim().split(',')) {
    ...
    if (build) result.push(...await load(path.join(__dirname, 'webpack', build)));
  }
